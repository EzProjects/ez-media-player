...
public class EzVideoPlayerProgressOverlay extends FrameLayout {
    ...
    public EzVideoPlayerProgressOverlay(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public EzVideoPlayerProgressOverlay(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void show(int delProgress, int curPosition, int duration) {
        ...
        if (delProgress > 0) {
            // 回退
            mSeekIcon.setImageResource(R.drawable.ico_ez_video_player_backward);
        } else {
            // 前进
            mSeekIcon.setImageResource(R.drawable.ico_ez_video_player_forward);
        }
