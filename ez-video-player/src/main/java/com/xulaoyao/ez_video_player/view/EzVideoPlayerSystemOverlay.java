 ...
 * 滑动变化音量|亮度框
 */
public class EzVideoPlayerSystemOverlay extends FrameLayout {

    private TextView mSystemTitle;
    private ImageView mSystemImage;
    private ProgressBar mProgressBar;

    public EzVideoPlayerSystemOverlay(Context context) {
        super(context);
        initialize(context);
    }

    public EzVideoPlayerSystemOverlay(Context context, AttributeSet attrs) {
        super(context, attrs);
        initialize(context);
    }

    private void initialize(Context context) {
        LayoutInflater.from(context).inflate(R.layout.ez_video_player_overlay_system, this);
    ...
    public void show(SystemType type, int max, int progress) {
        if (type == SystemType.BRIGHTNESS) {
            ...
            mSystemImage.setImageResource(R.drawable.ico_ez_video_player_system_ui_brightness);
        } else if (type == SystemType.VOLUME) {
            mSystemTitle.setText("音量");
            mSystemImage.setImageResource(progress == 0
                    ? R.drawable.ico_ez_video_player_system_ui_no_volume
                    : R.drawable.ico_ez_video_player_system_ui_volume);
        }
